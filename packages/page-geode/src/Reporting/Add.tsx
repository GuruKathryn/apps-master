...
import JSONgeodeReporting from './geode_suspicious_activity_reporting.json';
...
function Add ({ defaultAddress, onClose }: Props): React.ReactElement {
  ...
  const _contractName: string = 'Geode Reporting';
  const { api } = useApi();
  ...
  const _abi: string | null = JSON.stringify(JSONgeodeReporting)
  ...
  return (
    <Modal
      header={t('Load the Geode Reporting Contract to your local browser: (just Click Load!)')}
      onClose={onClose}
    >
      <Modal.Content>
      {isTestInfo && (
