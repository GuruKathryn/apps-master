...
import ViewReportsDetails from './ViewReportsDetails';
import ViewAllowedDetails from './ViewAllowedDetails';
...
function CallCard ({ className = '', contract, messageIndex, onCallResult, onChangeMessage, onClose }: Props): React.ReactElement<Props> | null {
  ...
  return (
    <Card >
        <h2><strong>{t<string>(' Geode Reporting ')}{' '}</strong>
        {messageIndex===0 && (
        ...
        {messageIndex !== null && messageIndex===7 && (
          <><br /><br />
          <Badge color='blue' icon='i'/>
          {t<string>('Only Legal Team Accounts Can See This:')}
          </>)}
        {!isCalled && (
        <>
        <InputAddress
