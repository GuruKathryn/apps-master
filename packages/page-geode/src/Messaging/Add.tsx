...
import JSONgeodeMessaging from './geode_messaging.json';
...
function Add ({ defaultAddress, onClose }: Props): React.ReactElement {
  ...
  const _contractName: string = 'Geode Private Messaging';
  const { api } = useApi();
  ...
  const _abi: string | null = JSON.stringify(JSONgeodeMessaging)
  ...
  return (
    <Modal
      header={t('Load the Geode Private Messaging Contract to your local browser: (just Click Load!)')}
      onClose={onClose}
    >
      <Modal.Content>
      {isTestInfo && (
