...
function MakeEndorsement(): JSX.Element {
    return(
        <div>
            ...
            {'  - '}{t<string>('You can either copy the claimId directly from the claim (or)')}<br />
            {'  - '}{t<string>('You can click on the grey claim box to get the claimId below')}<br />
            ...
            <strong>{t<string>('Claim to Endorse: ')}</strong>{hexToString(claimDetail.ok[isIndex].claim)}<br />
            <strong>{t<string>('ClaimID to Endorse:')}</strong>{t<string>(' (copy this Hash to the claimHash below:)')}
            <Output
                  className='output'
                  //isError={!result.isOk}
                  //isSmall
